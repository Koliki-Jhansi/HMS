...
async function seedDatabase(disconnectAfter = true) {
    try {
        ...
        await Promise.all([
            ...
            models_1.Patient.deleteMany({}),
            models_1.Doctor.deleteMany({}),
            models_1.DoctorSchedule.deleteMany({}),
            models_1.Department.deleteMany({}),
            models_1.Ward.deleteMany({}),
            models_1.Bed.deleteMany({}),
            models_1.Admission.deleteMany({}),
            models_1.Appointment.deleteMany({}),
            models_1.Prescription.deleteMany({}),
            models_1.MedicalRecord.deleteMany({}),
            models_1.Notification.deleteMany({}),
        ]);
        console.log('✅ Cleaned all collections.');
        const adminPassword = await bcryptjs_1.default.hash('admin123', 10);
        const doctorPassword = await bcryptjs_1.default.hash('doctor123', 10);
        const patientPassword = await bcryptjs_1.default.hash('patient123', 10);
        // 1. Seed Departments
        ...
        const departments = await models_1.Department.insertMany(deptsData);
        const deptMap = {};
        ...
        const wards = await models_1.Ward.insertMany(wardsData);
        const wardMap = {};
        wards.forEach((w) => {
            wardMap[w.code] = w;
        });
        ...
        const beds = await models_1.Bed.insertMany(bedsData);
        const bedMap = {};
        beds.forEach((b) => {
        ...
        for (const item of doctorsData) {
            const u = await models_1.User.create(item.user);
            const d = await models_1.Doctor.create({
                userId: u._id,
                ...item.docInfo,
            });
        ...
        for (const item of patientsData) {
            const u = await models_1.User.create(item.user);
            const p = await models_1.Patient.create({
                userId: u._id,
                ...item.patInfo,
            });
        ...
        await models_1.Admission.create({
        ...
        const apt1 = await models_1.Appointment.create({
        ...
        await models_1.Prescription.create({
